...
import com.davigueras.jfxh2h01.model.Product;
import com.davigueras.jfxh2h01.model.Validation;
...
public class MainController {
	...
	void performAction(ActionEvent event) {
		...
		case "new":
			...
			if (validRef && validName && validIssueDate && validCost && validProductionMinutes) {
				...
				String ref1 = fieldRef.getText();
				addingProduct.setRef(ref1);
				addingProduct.setName(fieldName.getText());
				addingProduct.setIssueDate(fieldIssueDate.getText());
				addingProduct.setCost(Double.valueOf(fieldCost.getText()));
				addingProduct.setProductionMinutes(Integer.valueOf(fieldProductionMinutes.getText()));

				Session session = HibernateUtil.getSessionFactory().openSession();
			    session.beginTransaction();  
			    session.persist(addingProduct); 
			    session.getTransaction().commit(); 
			    session.close();   
			    
				tableData.add(addingProduct); ////////////////////////////axo sha de quedar aixi?
	    		labelError.getStyleClass().add("green");
				labelError.setText("El Producto ha sido creado correctamente.");
				System.out.println("Product Id " + addingProduct.getId() + " created.");
				endAction();
			} else {
		...
		case "edit":
			...
			if (validRef && validName && validIssueDate && validCost && validProductionMinutes) {
				...
				fieldProductionMinutes.getStyleClass().remove("error");	
				fieldProductionMinutes.getStyleClass().remove("ok");
				Product editingProduct = tableData.get(index.get());
				editingProduct.setRef(fieldRef.getText());
				editingProduct.setName(fieldName.getText());
				editingProduct.setIssueDate(fieldIssueDate.getText());
				editingProduct.setCost(Double.valueOf(fieldCost.getText()));
				editingProduct.setProductionMinutes(Integer.valueOf(fieldProductionMinutes.getText()));
				
				Session session = HibernateUtil.getSessionFactory().openSession(); 
				session.beginTransaction();  
			    session.update(editingProduct); 
			    session.getTransaction().commit(); 
			    session.close();   
			    
				labelError.getStyleClass().add("green");
				labelError.setText("El Producto ha sido modificado correctamente.");
	    		System.out.println("Product Id " + editingProduct.getId() + " updated.");
		...
		case "delete":
			...
			Product deletingProduct = tableData.get(index.get());
	...
	private void initialize() {
	    ...
	    try { 
			...
			productsList = session.createQuery("from Product").list(); // Esta query es creada en HQL (hibernate query language)
	    } finally { 
	        session.close(); 
	    }  
	    
	    productsList.iterator();
